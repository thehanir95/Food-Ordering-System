      ...
      <ul class="navbar-nav ml-auto">
		...
		<li class="nav-item">
          ...
          <?php
			...
			{
			?>
			<a href="foods.php"><span style="color:green; font-size:30px;"><i class="fa fa-shopping-cart" aria-hidden="true"><span style="color:green;" id="cart"  class="badge badge-light"><?php if(isset($re)) echo $re; ?></span></i></span></a>
...
<br><br>
<div class="middle" style="  padding:60px; border:1px solid;  width:100%;">
       <!--tab heading-->
	   <ul class="nav nav-tabs nabbar_inverse" id="myTab" style="background:green;border-radius:10px 10px 10px 10px;" role="tablist">
          <li class="nav-item">
             <a class="nav-link active" style="color:black;" id="viewitem-tab" data-toggle="tab" href="#viewitem" role="tab" aria-controls="viewitem" aria-selected="true">View Cart</a>
          </li>
          <li class="nav-item">
		  ...
		  <li class="nav-item">
              <a class="nav-link" style="color:black;" id="orders-tab" data-toggle="tab" href="#orders" role="tab" aria-controls="orders" aria-selected="false">Orders</a>
          </li>
                 ...
                 <table class="table">
                  <tbody>
	               <?php
	                ...
	                ?>
					<tr>
							<th style=text-align:center;>Food View</th>
							<th style=text-align:center;>Food Name</th>
							<th style=text-align:center;>Price</th>
							<th style=text-align:center;>Ingredians</th>
							<th style=text-align:center;>Food Cabin Name</th>
							<th style=text-align:center;>Status</th>
					
					</tr>
                      <tr>
                         <td style=text-align:center;><image src="../image/restaurant/<?php echo $em."/foodimages/".$res['fldimage'];?>" height="80px" width="100px"></td>
                         <td style=text-align:center;><?php echo $res['foodname'];?></td>
                         <td style=text-align:center;><?php echo "RS ".$res['cost'];?></td>
                         <td style=text-align:center;><?php echo $res['cuisines'];?></td>
                         <td style=text-align:center;><?php echo $nm; ?></td>
		                <form method="post" enctype="multipart/form-data">
                           <td style=text-align:center;><button type="submit" name="del"  value="<?php echo $res['fld_cart_id']?>" class="btn btn-danger">Delete</button></td>
                        </form>
                   ...
                   <?php
					  ...
					  <td>
					  <h5><i></i>Rs.&nbsp;<?php echo end($gtotal); ?></h5>
					  </td>
					  <td>
	                 ...
	                 ?>
					 <tr><button type="button" class="btn btn-outline-success"><a href="foods.php" style="color:green; text-decoration:none;">No Items In cart Let's Shop Now</a></button></tr>
...
<!--tab 1 ends-->	   
            ...
            <div class="tab-pane fade" id="orders" role="tabpanel" aria-labelledby="orders-tab">
				...
				<th style=text-align:center;>Order status</th>
				    <tbody>
					<?php
					$quer_res=mysqli_query($con,"select * from tblorder where fld_email_id='$cust_id'");
					while($roww=mysqli_fetch_array($quer_res))
					{   
				         $fid=$roww['fld_food_id'];
						 $fcname=$roww['fldvendor_id'];

				         $qr=mysqli_query($con,"select * from tbfood where food_id='$fid'");
						 $qs=mysqli_query($con,"select * from tblvendor where fldvendor_id='$fcname'");
						 $qrr=mysqli_fetch_array($qr);
						 $qss=mysqli_fetch_array($qs);
						
					  
					?>
					   <tr>
					   <td style=text-align:center;><?php echo $qss['fld_name'];?></td>
					   <td style=text-align:center;><?php echo $roww['fld_order_id']; ?></td>
					   <?php
					   if(empty($qrr['foodname']))
					   {
					   ?>
					   <td style=text-align:center;><span style="color:red;"><b>Product Not Available Now</b></span></td>
					   <?php
					   }
					   else
					   {
						   ?>
						    <td style=text-align:center;><?php echo $qrr['foodname']; ?></td>
						   <?php
					   }
					   ?>
					   ...
					   {
						   ?>
						    <td style=text-align:center;><?php echo $qrr['cost']; ?></td>
						   <?php
					   }
					   ?>
					   <td style=text-align:center;><a href="#" onclick="del(<?php echo $roww['fld_order_id'];?>);"><button type="button" class="btn btn-danger">Cancel Order</button></a></td>
					   <td style=text-align:center;><b><?php echo $roww['fldstatus'];?></b></td>
					   </tr>
					 <?php
					}
					 ?>  
